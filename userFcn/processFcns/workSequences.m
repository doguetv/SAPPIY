...
%but unused.
%% Comments
%The maxSequences function finds maximum value of each signal sequences and
%associate a marker (line object) element to each signal sequence.

%Associated GUI: SAPPIY
%Author: V. Doguet (3/5/2019)
%Updates:
%% Main Function
function [results, newElements] = workSequences(axis, data, results, callbacks)
...
for i = 1:size(data.sequences, 1)
    ...
    work = trapz(data.xData(i, :), data.sequences(i, :));
    %Store in results variable
    results{i, labelColumn} = 'Work Amount';
    results{i, valueColumn} = work;
    %Store new elements to associate with results
    %Element type must be
    %defined
    newElements{i, 1}.type = 'area(0)';
    %Then other fields must correspond to valid properties of defined
    %object type.
    newElements{i, 1}.XData = data.xData(i, :);
    newElements{i, 1}.YData = data.sequences(i, :);
    newElements{i, 1}.FaceColor = 'g';
    newElements{i, 1}.EdgeColor = 'k';
    newElements{i, 1}.LineStyle = '--';
    newElements{i, 1}.BaseValue = min(axis.YLim);
end
end